defmodule MobMishka.Components.MishkaJsonInput do
  @moduledoc """
  ...
  it is a question about *one field*. This is the handle for asking it.
      ...
      iex> MobMishka.Components.MishkaJsonInput.error_id("config")
      "config-error"
  """
  ...
  and the web component does not paint one red either.
      ...
      iex> MobMishka.Components.MishkaJsonInput.validate(~s({"a": 1}))
      {:ok, %{"a" => 1}}

      iex> MobMishka.Components.MishkaJsonInput.validate("   ")
      :empty

      iex> match?({:error, _}, MobMishka.Components.MishkaJsonInput.validate("{oops}"))
      true

  A bare scalar is valid JSON, and treating it as an error would be wrong:

      iex> MobMishka.Components.MishkaJsonInput.validate("42")
      {:ok, 42}
  """
  ...
  reformatting as someone types would move the cursor out from under them.
      ...
      iex> MobMishka.Components.MishkaJsonInput.format(~s({"a":1}))
      "{\\n  \\"a\\": 1\\n}"

      iex> MobMishka.Components.MishkaJsonInput.format("{oops}")
      "{oops}"
  """
  ...
  otherwise the parse decides.
      ...
      iex> MobMishka.Components.MishkaJsonInput.invalid?(%{}, {:error, "nope"})
      true

      iex> MobMishka.Components.MishkaJsonInput.invalid?(%{}, :empty)
      false

      iex> MobMishka.Components.MishkaJsonInput.invalid?(%{invalid: true}, {:ok, 1})
      true
  """
