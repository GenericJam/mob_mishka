defmodule MobMishka.Components.MishkaContextMenu do
  @moduledoc """
  Native Mob port of Mishka Chelekom's **headless Context Menu** — the actions
  for a particular row or object.

  Renders through `MobMishka.Components.MishkaMenu`, because a context menu and a
  ...
  (`MobMishka.Components.Anchored`) does draw a panel over the page against a
  trigger's own box, and `MobMishka.Components.MishkaPopover` is built on it.
  This renders through `MobMishka.Components.MishkaMenu`, which has not been
  ...
  Everything `MobMishka.Components.MishkaMenu` accepts, plus:
