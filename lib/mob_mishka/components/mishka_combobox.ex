defmodule MobMishka.Components.MishkaCombobox do
  @moduledoc """
  ...
  `MobMishka.Components.MishkaSelect`'s job, so the option list and the
  ...
  estimate `MobMishka.Components.MishkaTagsInput` uses — nothing here can measure
  text.
  ...
  `:anchored` node (`MobMishka.Components.Anchored`), which
  `MobMishka.Components.MishkaPopover` is built on — but its window dismisses
  ...
  alias MobMishka.Components.{Event, MishkaMenu, MishkaPill, MishkaSelect, MishkaTagsInput}
  ...
  make a list unsearchable. An empty query matches everything.
      ...
      iex> alias MobMishka.Components.MishkaCombobox, as: C
  ...
  Case-fold and strip diacritics, so "Café" and "cafe" compare equal.
      ...
      iex> MobMishka.Components.MishkaCombobox.fold("Café")
      "cafe"
      iex> MobMishka.Components.MishkaCombobox.fold(nil)
      ""
  """
