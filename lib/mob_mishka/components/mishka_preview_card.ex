defmodule MobMishka.Components.MishkaPreviewCard do
  @moduledoc """
  ...
  `MobMishka.Components.MishkaPopover`, so holding the trigger a second time can
  ...
  The pair is an `:anchored` node now (`MobMishka.Components.Anchored`). Child
  ...
  sharing a parent with it:
    ...
    * The card hugs rather than filling. Inside a popup window "fill" is the
      ...
      `MobMishka.Components.MishkaPopover.panel/2` honours an explicit value.
  ...
  | plus everything `MobMishka.Components.MishkaPopover`'s **panel** accepts | | | `width`, `background`, `corner_radius`, `padding`, `border_*`, `fill_width`. |
  ...
  alias MobMishka.Components.{Anchored, Event, MishkaAvatar, MishkaPopover}
